...
class Profileimage extends BaseController
{
    ...
    public function update()
    {
        ...
        if (! $file->isValid()){
            ...
            if ($error_code == UPLOAD_ERR_NO_FILE){
                return redirect()->back()
                                ->with('warning', lang('Profile.nofile'));
            }
        ...
        if ($size > 2){
            return redirect()->back()
                            ->with('warning', lang('Profile.filetoo'));
        }
        ...
        if (! in_array($type, ['image/png', 'image/jpeg'])){
            ...
            return redirect()->back()
                            ->with('warning', lang('Profile.invalidfile'));
        }
        ...
        return redirect()->to("/profile/show")
                        ->with('info', lang('Profile.imageupload'));
    ...
    public function delete()
    {
        if ($this->request->getMethod() === 'post'){
            ...
            return redirect()->to('/profile/show')
                            ->with('info', lang('Profile.imagedelete'));
