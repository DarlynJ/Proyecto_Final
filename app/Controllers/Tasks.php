...
class Tasks extends BaseController
{
    ...
    public function create(){
        ...
        if ($this->model->insert($task)){
            ...
            return redirect()->to("/tasks/show/{$this->model->insertID}")
                            ->with('info', lang('Tasks.taskcreated'));
        }else{
            return redirect()->back()
                             ->with('errors', $this->model->errors())
                             ->with('warning', lang('Tasks.invalid'))
                             ->withInput();
        }
    }
    ...
    public function update($id){
        ...
        if ( ! $task->hasChanged()){
            return redirect()->back()
                            ->with('warning', lang('Tasks.nothing'))
                            ->withInput();
        }
        ...
        if ($this->model->save($task)){
            ...
            return redirect()->to("/tasks/show/$id")
                            ->with('info', lang('Tasks.taskupdated'));
        } else{
    ...
    public function delete($id){
       ...
       if ($this->request->getMethod()==='post'){
        ...
        return redirect()->to('/tasks')
                        ->with('info', lang('Tasks.taskdelete'));
       }
    ...
    private function getTaskOr404($id)
    {
        ...
        */
        ...
        $task = $this->model->getTasksByUserId($id, $this->current_user->id);
