...
class Users extends \App\Controllers\BaseController
{
    ...
    public function create(){
         ...
         if ($this->model->protect(false)
            ...
            return redirect()->to("/admin/users/show/{$this->model->insertID}")
                            ->with('info', lang('Userr.usercreated'));
        }else{
            return redirect()->back()
                             ->with('errors', $this->model->errors())
                             ->with('warning', lang('Userr.invalid'))
                             ->withInput();
        }
    }
    ...
    public function update($id){
        ...
        if ( ! $user->hasChanged()){
            return redirect()->back()
                            ->with('warning', lang('Userr.nothing'))
                            ->withInput();
        }
        ...
        if ($this->model->protect(false)
            ...
            return redirect()->to("/admin/users/show/$id")
                            ->with('info', lang('Userr.userupdated'));
        } else{
    ...
    public function delete($id){
       ...
       if ($this->request->getMethod()==='post'){
        ...
        return redirect()->to('/admin/users')
                        ->with('info', lang('Userr.userdelete'));
       }
